...
public class PreferenciaService {
    ...
    public PreferenciaResponseDTO createPreferencia(UUID idUsuario, PreferenciaRequestDTO dto) {
        ...
        if (!preferencia.evento.personalizavel) {
            throw new ForbiddenException(
                    String.format("Não é possível criar preferências para o evento '%s' pois ele não é personalizável.",
                            preferencia.evento.nomeEvento)
            );
        }
    ...
    public void substituirPreferenciasDoUsuario(Usuario usuario, List<PreferenciaRequestDTO> dtos) {
        ...
        for (PreferenciaRequestDTO dto : dtos) {
            ...
            if (Boolean.TRUE.equals(dto.getPersonalizavel()) && !evento.personalizavel) {
                throw new ForbiddenException("O evento " + evento.nomeEvento + " não aceita parâmetros personalizados, apenas monitoramento padrão.");
            }
