...
def analyze(self, input_file: str, execution_id=None):
    ...
    _log_safe(execution_id, "Análise Iniciada", STATUS_IN_PROGRESS, task=TASK_NAME_ANALYZE)
    ...
    with Timer() as timer:
        ...
        try:
            ...
            _log_safe(execution_id, "Análise Finalizada", STATUS_SUCCESS, task=TASK_NAME_ANALYZE)
            ...
            _log_safe(
                execution_id, "Erro na Análise", STATUS_ERROR, extra={"error": str(e)}, task=TASK_NAME_ANALYZE
            )
            logger.exception(
...
def transform(self, input: str, output: str, execution_id=None):
    ...
    execution_id = execution_id or self.request.id
    _log_safe(execution_id, "Transform Iniciado", STATUS_IN_PROGRESS, task=TASK_NAME_TRANSFORM)
    ...
    with Timer() as timer:
        ...
        try:
            ...
            _log_safe(execution_id, "Transform Finalizado", STATUS_SUCCESS, task=TASK_NAME_TRANSFORM)
            ...
            _log_safe(
                execution_id, "Erro no Transform", STATUS_ERROR, extra={"error": str(e)}, task=TASK_NAME_TRANSFORM
            )
            logger.exception(
...
def download_file(self):
    ...
    _log_safe(execution_id, "Task Iniciada", STATUS_STARTED, task=TASK_NAME_DOWNLOAD_FILE)
    _log_safe(execution_id, "Download Iniciado", STATUS_IN_PROGRESS, task=TASK_NAME_DOWNLOAD_FILE)
    ...
    with Timer() as timer:
        ...
        try:
            ...
            _log_safe(execution_id, "Download Finalizado", STATUS_SUCCESS, task=TASK_NAME_DOWNLOAD_FILE)
