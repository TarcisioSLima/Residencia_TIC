...
EventType = Literal["temperatura alta", "temperatura baixa", "umidade baixa", "vento", "chuva", "sensação térmica"]
...
def get_severity(tipo: str, valor: float) -> SeverityInfo:
    ...
    if tipo_lower == "sensação térmica":
        if 32 <= valor < 41:
            return SeverityInfo("#f39c12", "🌡️", "Atenção — sensação térmica elevada (32°C – 41°C).")
        if 41 <= valor < 54:
            return SeverityInfo("#e74c3c", "🌡️", "Perigo extremo — risco à saúde (41°C – 54°C).")
        if valor >= 54:
            return SeverityInfo("#c0392b", "🌡️", "Perigo — emergência de calor (acima de 54°C).")
        return SeverityInfo("#7f8c8d", "🌡️", "Sensação térmica dentro da normalidade.")
